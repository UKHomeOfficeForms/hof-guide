...
You may want create a custom validator and display custom validation messages when it fails. To do this, do not include your validator
in the fields/index.json as normal, rather create a custom controller:

````
class IncomeController extends BaseController {
  validateField(keyToValidate, req) {
    var error;
    var salaryKey = 'salaryAmount';
    if (keyToValidate === salaryKey &&
      !(req.form.values[salaryKey] > 0)) {
      error = new this.ValidationError(`${keyToValidate}`, {
        key: salaryKey,
        type: 'greaterThanZero', // line A
        redirect: null
      });
    }
    return error ? error : super.validateField.apply(this, arguments);
  }
}
````
You can add a number of conditions to your controller. If your conditions fails, return an object containing the validation type,
as on line A.

Add this controller to the step you want to validate in index.js:
````
'/income': {
  fields: [
    'incomeTypes'
...
Finally add a custom validation message to match your validation type in validation.json:
````
"salaryAmount": {
    "default": "Enter total salary amount per month",
    "greaterThanZero": "Salary amount must be greater than zero"
}
````
